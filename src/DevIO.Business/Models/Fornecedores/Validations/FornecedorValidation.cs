    ...
    public class FornecedorValidation : AbstractValidator<Fornecedor>
    {
        public FornecedorValidation()
        {
            RuleFor(f => f.Nome)
                ...
                .Length(2, 200)
                .WithMessage("O campo {PropertyName} precisa ter entre {MinLenght} e {MaxLenght} caraceteres");

            When(f => f.TipoFornecedor == TipoFornecedor.PessoaFisica, () =>{

                RuleFor(f => f.Documento.Length).Equal(CpfValidacao.TamanhoCpf)
                                                .WithMessage("O campo {PropertyName} precisa ter {Comparison} caracteres e foi fornecido {ProperytValue}");
